...
geomodelgrids::serial::TestCQuery::testAccessors(void) {
    ...
    err = geomodelgrids_squery_setSquashing(handle, false);CPPUNIT_ASSERT(!err);
...
// Test queryElevation().
void
geomodelgrids::serial::TestCQuery::testQueryElevation(void) {
    const size_t numModels = 2;
    const char* const filenames[numModels] = {
    ...
    { // One Block Flat
        ...
        for (size_t iPt = 0; iPt < numPoints; ++iPt) {
            ...
            const double elevation = geomodelgrids_squery_queryElevation(handle, pointsLLE[iPt*spaceDim+0],
    ...
    // Bad handle
    double elevation = geomodelgrids_squery_queryElevation(NULL, 0.0, 0.0);
...
// Test query() for model with topography using squashing.
void
geomodelgrids::serial::TestCQuery::testQuerySquash(void) {
